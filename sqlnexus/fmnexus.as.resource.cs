...
namespace sqlnexus
{
    ...
    public partial class fmNexus : Form, ILogger
    {
        ...
        public void LogMessage(string msg, MessageOptions options, TraceEventType eventtype)
        {
            ...
            Trace.Flush();
            if ((options & MessageOptions.Dialog) == MessageOptions.Dialog)
        ...
        void PopulateDatabaseList(String CurDB)
        {
            ...
            SqlDataAdapter da = new SqlDataAdapter("select name  from sys.databases where database_id > 4 and source_database_id is null and (name not like 'ReportServer%') order by name", Globals.ConnectionString);
...
////                        UpdateServiceButtons();
        ...
        public void EnumReports()
        {
            ...
            foreach (string rp in Globals.resourceReport.Keys)
            {
                AddFindReportNode(rp, null);
            }
        ...
        */
                ...
                try
                {
                    ...
                    foreach (KeyValuePair<DataTable, string> kvp in reportDataTables)
                    {
                        if (0 == string.Compare(kvp.Value, reportname, true, CultureInfo.InvariantCulture))
                        {
                            ...
                            string qrytext = fmNexus.GetQueryText((report.ReportPath==null?report.DisplayName:report.ReportPath), report.GetParameters(), DataSetName);

                            if (0 == qrytext.Length)
                                qrytext = DataSetName + " " + GetProcParams((report.ReportPath == null ? report.DisplayName : report.ReportPath), report.GetParameters(), DataSetName);
        ...
        {
            XmlDocument doc = new XmlDocument();
            if (Globals.resourceReport.ContainsKey(filename))
                doc.LoadXml(Globals.resourceReport[filename].reportDefinition);
            else
                doc.Load(filename);
                ...
                foreach (ReportParameterInfo p in paramc)
                {
                    ...
                    {   // param has an existing value
                        switch (p.DataType)
                        {
                            case ParameterDataType.DateTime:
                                {
                                    paramstr = "'" + Convert.ToDateTime(p.Values[0]).ToString("yyyy-MM-ddTHH:mm:ss.fff") + "'";
                                    break;
                                }
                            case ParameterDataType.String:
                                {
        ...
        private void FixupDataSources(string filename, string reportname, ReportDataSourceCollection datasources, ReportParameterInfoCollection paramc)
        {
            try
            {
                ...
                XmlDocument doc = new XmlDocument();
                if (Globals.resourceReport.ContainsKey(reportname))
                    doc.LoadXml(Globals.resourceReport[reportname].reportDefinition);
                else
                    doc.Load(filename);
        ...
        private static void SetReportQueryParameters(LocalReport report)
        {
            ...
            if (report.ReportPath == null)
                doc.LoadXml(Globals.resourceReport[report.DisplayName].reportDefinition);
            else
            ...
            foreach (XmlNode node in nodes)
            {
                ...
                if (null != dsetnode)  //value from dataset
                {
                    ...
                    //need to handle resource report
                    SqlDataAdapter da = new SqlDataAdapter(fmNexus.GetQueryText((report.ReportPath==null? report.DisplayName:report.ReportPath), report.GetParameters(), dsetnode.InnerText), Globals.ConnectionString);
                    da.Fill(dt);
            ...
            try
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        if (Globals.resourceReport.ContainsKey(reportpath))
                        {
                            report.LoadReportDefinition(new StringReader(Globals.resourceReport[reportpath].reportDefinition));
                        }
        ...
        private bool CheckAndRegisterService()
        {
            ...
            try
            {
                ...
                {
                    if (!Util.ConnectingtoCurrentMachine(Globals.Server))  //Can't register service on remote machine
                    {
                    ...
                    string regparams = "/R /I SQLNexusRealtime.xml /P \"" + Application.StartupPath + "\\Collection\" /O \"" + Application.StartupPath + string.Format("\\Collection\\Output\" /Vsqlnexusdb={0} /Asqlnexus",Globals.Database);
        ...
        private bool CreateDB(String dbName)
        {
            ...
            {
                success = false;
                LogMessage(String.Format(Properties.Resources.Error_Nexus_CreateDBFailure, dbName, Globals.Server, sqlEx.Message), MessageOptions.All, TraceEventType.Error);
            }
            finally
            {
                conn.Close();
        ...
        private void tscCurrentDatabase_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                String CurrentDatabase = Globals.Database;
            ...
            {
                Globals.Database = tscCurrentDatabase.SelectedItem.ToString();
                CloseAll();
                EnumReports();
                if (0 != tvReports.Nodes.Count)
                    tvReports.SelectedNode = tvReports.Nodes[0];
